...
def main():
    ...
    do_predictions(model_type=model_type)
...
def print_metrics(y_pred,y_list,y_logit_list,classes,model_type):
    if(model_type==MODEL_TYPES[1]):
        ...
        print(y_pred.shape)
        approx_acc_1=approximate_accuracy(y_list_index,y_pred_index,y_pred.shape[-1],1)
        approx_acc_2=approximate_accuracy(y_list_index,y_pred_index,y_pred.shape[-1],2)
        approx_acc_3=approximate_accuracy(y_list_index,y_pred_index,y_pred.shape[-1],3)
        approx_acc_4=approximate_accuracy(y_list_index,y_pred_index,y_pred.shape[-1],4)
        approx_acc_5=approximate_accuracy(y_list_index,y_pred_index,y_pred.shape[-1],5)        
