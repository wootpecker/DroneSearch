...
def choose_model(model_type=MODELS[0], output_shape=1, device="cuda", input_shape=1):
  ...
  if(model_type==MODELS[0]):
    model = VGG(output_shape=output_shape,input_shape=input_shape).to(device)
  ...
  print(f"[BUILDER] Model (Type: {model_type}, Classes: {output_shape}, Device: {device}) loaded.")
  return model
...
class VGGVariation(nn.Module):
    ...
    def __init__(self, output_shape: int, input_shape=1) -> None:
        ...
        self.classifier = nn.Sequential(
          nn.Flatten(),
          nn.Linear(in_features=FEATURE_MAP[1]*16*16, out_features=output_shape)
        )
